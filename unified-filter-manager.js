...
class UnifiedFilterManager {
  constructor(dataProcessor, chartVisualizer) {
    ...
    this.startX = 0;
    this.startWidth = 0;
    this.resizeBoundMouseMove = this.handleResizeMove.bind(this);
    this.resizeBoundMouseUp = this.handleResizeUp.bind(this);
  ...
  initializeFilters() {
    ...
    // Add resize handle
    this.createResizeHandle(filterContainer);

    // Get available filter options
    const filters = this.dataProcessor.getFilters();

    // --- Time Period Section ---
    this.createSectionTitle(filterContainer, 'Time Period');
    this.createDateRangeFilter(filterContainer);

    // --- Campaign Details Section ---
    this.createSectionTitle(filterContainer, 'Campaign Details');
    ...
    // --- Targeting Section ---
    this.createSectionTitle(filterContainer, 'Targeting');
    ...
    // --- Technical Section ---
    this.createSectionTitle(filterContainer, 'Technical');
    ...
    // --- Actions ---
    this.createSectionTitle(filterContainer, 'Actions');
  ...
  /**
   * Create a section title for grouping filters
  ...
  /**
   * Create and initialize the resize handle
   * @param {HTMLElement} container 
   */
  createResizeHandle(container) {
    const handle = document.createElement('div');
    handle.className = 'filter-resize-handle';
  ...
  handleResizeMove(e) {
    ...
    // Constrain width (min 200px, max 600px)
  ...
  handleResizeUp() {
    this.isResizing = false;
    const handle = document.querySelector('.filter-resize-handle');
